...
- **A1 · Ship-the-Model flavored choice** *(P1, M)* — GDD §4 promises deploy / open-source / sell-to-hyperscaler as a player-flavored choice with minor different bonuses (e.g. open-source → +Reputation, sell → +Money kickstart, deploy → +Legacy). Today it's a single "Ship" button. Add 3 options at the prestige moment; pure fold in `prestige.ts`, data in balance, gated so default path = current curve.
- **A2 · Audio: ambient bed + era stingers** *(P1, M)* — only SFX exist; GDD §6 calls for "music swells" at era/ascension moments. Add a low synthesized ambient pad (Web Audio, no assets) + a stinger on era transition / ship / ascension, all under the existing Sound toggle.
- **A3 · First-session hook polish** *(P1, M)* — tighten the first 60–90s: clearer first goal ("train your first model"), a guided first upgrade, first claim juice, and the first prestige tease. The onboarding exists but the *pull* into the loop is the retention lever.
- **A4 · "Manifestation rule" audit** *(P1, M)* — GDD §5's load-bearing dopamine: confirm every buyable thing that *can* be physical shows in the hall (racks slide in, cooling animates, eras reskin). Fill any gaps where an upgrade changes a number but not the room.
- **A5 · Settings: save export/import + reset confirm** *(P1, S)* — local-only is fine (no backend per GDD), but a copy-to-clipboard export/import string protects players from losing a long run, and a guarded reset avoids accidents. Big trust win, tiny cost.
