...
export type LabSection = "build" | "research" | "hq";

export interface AdvisorItem {
  /** Which tab resolves this item (where tapping the nudge should jump). */
  tab: AdvisorTab;
  /** For lab items: the Lab sub-section that resolves it. */
  section?: LabSection;
...
export function advisorItems(state: GameState): AdvisorItem[] {
  ...
  if (productsUnlocked(state)) {
    ...
    if (ps.active.length === 0 && ps.drafts.length === 0) {
      items.push({ tab: "lab", section: "hq", text: "Ship a model in the Lab to start a product", priority: 80 });
    }
  }
